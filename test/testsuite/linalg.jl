function test_linalg(AT)
    @testset "linear algebra" begin
        ...
        @testset "$T gemm y := $f(A) * x * a + y * b" for f in (identity, tranpose, adjoint), T in supported_eltypes()
        ...
        @testset "$T gemm C := $f(A) * $g(B) * a + C * b" for f in (identity, tranpose, adjoint), g in (identity, transpose, adjoint), T in supported_eltypes()
